...
instance Print (Backend.X64.Parser.Gen.AbsXGAS.AsmInstr' a) where
  prt i = \case
    ...
    Backend.X64.Parser.Gen.AbsXGAS.IDIV64 _ target -> prPrec i 0 (concatD [doc (showString "idivq"), prt 64 target])
    Backend.X64.Parser.Gen.AbsXGAS.NEG32 _ target -> prPrec i 0 (concatD [doc (showString "negl"), prt 32 target])
    Backend.X64.Parser.Gen.AbsXGAS.IDIV32 _ target -> prPrec i 0 (concatD [doc (showString "idivl"), prt 32 target])
    Backend.X64.Parser.Gen.AbsXGAS.NEG16 _ target -> prPrec i 0 (concatD [doc (showString "negb"), prt 16 target])
    Backend.X64.Parser.Gen.AbsXGAS.IDIV16 _ target -> prPrec i 0 (concatD [doc (showString "idivb"), prt 16 target])
    Backend.X64.Parser.Gen.AbsXGAS.CALL _ label -> prPrec i 0 (concatD [doc (showString "CALL"), prt 0 label])
    Backend.X64.Parser.Gen.AbsXGAS.CALLINDIRECT _ n reg -> prPrec i 0 (concatD [doc (showString "CALL"), doc (showString "*"), prt 0 n, doc (showString "("), prt 64 reg, doc (showString ")")])
