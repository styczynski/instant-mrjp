...
runBackend :: String -> String -> (IR.Program IR.IRPosition) -> LatteBackend -> LattePipeline (Either Errors.Error (String, String))
runBackend filePath fileName ast backend = do
  p <- runExceptT (runBackend' filePath fileName ast backend)
  return p
  where
    runBackend' :: String -> String -> (IR.Program IR.IRPosition) -> LatteBackend -> BackendPipeline (String, String)
    runBackend' filePath fileName ast backend = do
