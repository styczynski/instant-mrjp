...
data Value = Constant (Syntax.Lit Position) | Dynamic | Marker
    deriving (Eq, Show)

data ConstPropagationEnv = ConstPropagationEnv {
    _cpVars :: [(Syntax.Ident Position, Value)]
    , _cpVarFreeIndex :: Int
}
...
zero (Syntax.IntT _) = Constant (Syntax.Int Undefined 0)
zero (Syntax.ByteT _) = Constant (Syntax.Byte Undefined 0)
zero (Syntax.BoolT _) = Constant (Syntax.Bool Undefined False)
zero _ = Constant (Syntax.Null Undefined)

run :: Syntax.Program Position -> Optimizer ConstPropagationEnv (Syntax.Program Position)
run prog = return prog
